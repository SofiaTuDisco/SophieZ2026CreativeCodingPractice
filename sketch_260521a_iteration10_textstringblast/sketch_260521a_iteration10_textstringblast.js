...
function preload() {
  ...
  blastText = loadStrings("data/textstringblast.txt");
  myFont= loadFont("data/CourierStd-Oblique.otf");
}
...
function draw() {
  ...
  let sizing = [25, 12, 34, 56, 18, 26, 24, 48, 64]; // an array of possible sizes (stack the odds!)
...
function keyPressed() {
  if (key === 's') {
    saveCanvas('bad_eyes_textstring_blast');
  }
}
