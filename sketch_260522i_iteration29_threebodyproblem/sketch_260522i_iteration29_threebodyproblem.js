...
function preload() {
  ...
  bgImage = loadImage("data/IMG_3478cathorizontal.JPG");  
}

function setup() {
  // The magic third argument, WEBGL, switches p5 into 3D mode.
  // Without it, box(), sphere() and lighting won't work.
  createCanvas(windowWidth, windowHeight, WEBGL); //<~~~~~~ WEBGL MODE ENABLED HERE YAY:o ;)
...
pop();
 ...
 // pop(); // Restore position/rotation before drawing the next shape
  ...
  push();
    translate(42, -42, 100); // Right, slightly up, and back into the scene
    // OPTIONAL ORBIT VERSION: comment out the line above and uncomment below.
    //translate(cos(frameCount * 0.02) * 460, 20, sin(frameCount * 0.02) * 40);
    noStroke();
    fill(255, 0, 0);
    sphere(30); // A sphere — the one argument is the radius
  pop();
  
    push();
    translate(220, -220, -210); // Right, slightly up, and back into the scene
    // OPTIONAL ORBIT VERSION: comment out the line above and uncomment below.
    //translate(cos(frameCount * 0.03) * -240, 20, sin(frameCount * 0.02) * 140);
    noStroke();
    fill(255, 0, 0);
    sphere(90); // A sphere — the one argument is the radius
  pop();
  
     push();
    translate(-200, 80, 100); // Right, slightly up, and back into these scene
    // OPTIONAL ORBIT VERSION: comment out the line above and uncomment below.
    //translate(cos(frameCount * 0.04) * 240, -20, sin(frameCount * 0.04) * 240);
    noStroke();
    fill(255, 0, 0);
    sphere(150); // A sphere — the one argument is the radius
  pop();
}
