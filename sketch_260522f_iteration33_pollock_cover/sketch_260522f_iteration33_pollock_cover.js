...
function preload() {
  bigImage = loadImage("data/layer2_1.png");
...
function setup() {
  ...
  background(bigImage); // Bright green canvas
}
...
function draw() {
  ...
  if (mouseIsPressed) {
    ...
    fill(pmouseY / 3, pmouseX / 3, (pmouseX + pmouseY) / 3);
  } else {
    // Normal color mapping based on current mouse position
    fill(mouseY / 1, mouseX / 1, (mouseX + mouseY) / 1);
  }
